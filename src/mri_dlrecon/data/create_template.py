...
def create_temp_data(path):
    ...
    for dataset in data_splits:
        for split in data_splits[dataset]:
            fcount = 0
            (Path(path) / dataset / split).mkdir(parents=True)
            encs = enc_sizes[split.split("_")[-1]]
            recs = recon_sizes[split.split("_")[-1]]
            for i in range(len(encs)):
                fname = Path(path) / dataset / split / f"file{fcount}.h5"
    ...
    return Path(path) / "brain_data1"


# brain_data_path = create_temp_data("src/mri_dlrecon/data")
